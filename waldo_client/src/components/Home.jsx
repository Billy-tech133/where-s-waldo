...
import bgImage2 from '../assets/game_img.png'
import { Link } from 'react-router-dom'
const Home = () => {
  return (
    <section>
      <header className="relative bg-cover h-100 w-full rounded-4xl shadow-2xl shadow-blue-300 -z-0" style={{ backgroundImage: `url(${bgImage2})` }}>
      ...
      <div className="relative bg-cover h-100 w-full rounded-4xl shadow-2xl shadow-blue-300 -z-0 translate-transform transform hover:scale-103" style={{ backgroundImage: `url(${bgImage2})` }}>
        <div className='absolute flex h-[100%] items-center inset-0 bg-black/40 rounded-4xl'>
            <div className='ml-8 w-[50%]'>
            <Link to="/game" className='fond-bold text-white text-5xl w-[50%] cursor-pointer hover:underline'>
                Click to play game.
            </Link>
            </div>
        </div>
